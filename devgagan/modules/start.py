...
from config import OWNER_ID, START_GIF
...
BOT_COMMANDS = [
    ...
    BotCommand("setchannel", "📡 Set upload destination channel"),
    BotCommand("removechannel", "🗑 Remove upload channel (back to DM)"),
...
async def register_bot_commands():
    try:
        ...
        print(f"⚠️  Could not register bot commands: {e}")
...
help_pages = [
    ...
    (
        ...
        "19. **/setchannel -100XXXXXXXXX**\n"
        "> Set the channel/group where all extracted files will be uploaded\n"
        "> Run `/setchannel` alone to see your current setting\n\n"
        "20. **/removechannel**\n"
        "> Remove the saved upload channel (files go back to your DM)\n\n"
...
async def start(client, message):
    if len(message.command) > 1:
        return  # deep-link /start <param> is handled by shrink.py
    ...
    buttons = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("📋 Commands", callback_data="help_next_0"),
    ...
    caption = (
        f"👋 **Hello, {name}!**\n\n"
        "🤖 I'm a **Telegram Media Extractor Bot**.\n"
        "I can save videos, photos, and documents from any Telegram channel "
        "— even protected ones (with /login).\n\n"
        "**📌 Quick start:**\n"
        "> 1️⃣ Just paste any `t.me/...` link to extract a single file.\n"
        "> 2️⃣ Use /batch to extract multiple files at once.\n"
        "> 3️⃣ Use /login to access private or protected channels.\n\n"
    ...
    for step in range(1, 5):
        shown = lines[: max(1, len(lines) * step // 4)]
        frames.append(f"{spinner[step - 1]} Loading...\n\n" + "\n".join(shown))

    try:
        msg = await asyncio.wait_for(
            message.reply_animation(animation=START_GIF, caption=frames[0]),
            timeout=15,
        )
        for frame in frames[1:]:
            await asyncio.sleep(0.7)
            await msg.edit_caption(frame)
        await asyncio.sleep(0.7)
        await msg.edit_caption(caption, reply_markup=buttons)
    except Exception as e:
        # GIF host slow or unreachable — plain text can never fail for network reasons
        print(f"Animated /start failed, falling back to text: {e}")
        try:
            await message.reply_text(caption, reply_markup=buttons)
        except Exception as e2:
            print(f"/start reply failed completely: {e2}")
